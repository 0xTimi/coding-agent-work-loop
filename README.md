...
> **Agent Skill** — 注册名 `coding-agent-work-loop`。当前打包格式针对 Claude Code（含 SKILL.md frontmatter + `~/.claude/skills/` 目录约定）；脚本主体是纯 shell + GitHub CLI，其他 agent 框架可以直接复用同一份代码（见下面 [自定义 worker](#自定义-worker不是-claude-code)）。
> 本文档里「项目」指 GitHub 仓库 `luosky/coding-agent-work-loop`；「skill」指它装到 agent 运行环境后的标识 `coding-agent-work-loop`。
...
链路：
```
~/.claude/skills/coding-agent-work-loop
   → ~/.agents/skills/coding-agent-work-loop
...
> **快速安装（不推荐，但能跑）**：直接 `git clone https://github.com/luosky/coding-agent-work-loop.git ~/.claude/skills/coding-agent-work-loop`。
...
4. `~/.config/coding-agent-work-loop/<key>.conf` 注册 systemd EnvironmentFile
...
~/.agents/skills/coding-agent-work-loop  -> ~/github/coding-agent-work-loop      # 规范 skill 单源
~/.claude/skills/coding-agent-work-loop -> ~/.agents/skills/coding-agent-work-loop # Claude Code 注册
```

> 快速安装的人直接把 repo clone 到 `~/.claude/skills/coding-agent-work-loop/`——结构相同，只是没有 symlink 这一步（不推荐：失去多 agent runtime 共享 + 代码混进 .claude/ 私有目录）。
...
```bash
cd ~/.agents/skills/coding-agent-work-loop
git pull
```
