...
**当前主要适配 Claude Code**：它从 `~/.claude/skills/<name>/` 自动发现 skill。但**项目
源码不建议直接 clone 进 `~/.claude/`**——那是 agent 的私有 config 目录，开源项目代
码混进去会跟其他东西搅在一起，也不利于 `git pull` 升级 / fork 改造 / 多 agent 框架复用。

**推荐做法**：项目 clone 到你平时放代码的标准位置，再 symlink 到 agent 期望的 skill 目录：
...
> 之后 `git pull ~/github/coding-agent-work-loop` 升级，agent 自动看到新版本——symlink 不需要重做。

> **非 Claude Code 用户**：daemon 和 dispatch 脚本是纯 shell + `gh` CLI，不依赖任何
> agent 框架。直接 `bash scripts/agent-poll.sh`（或交给 cron / systemd 调起）即可。
> SKILL.md 仅用于 Claude Code 把这份 skill 加载到上下文，其他场景可以无视。
...
# symlink 进 agent 期望的 skill 目录（Claude Code 默认是 ~/.claude/skills/<name>/）
mkdir -p ~/.claude/skills
ln -s ~/github/coding-agent-work-loop ~/.claude/skills/coding-agent-workflow
```
