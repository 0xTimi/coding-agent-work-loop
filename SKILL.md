...
> This skill is intentionally agent-agnostic: the daemon and dispatch scripts are plain shell + `gh` CLI, so **any AI CLI that can be launched inside tmux and accept a stdin prompt can act as the worker**. The default worker is `claude` (Claude Code); swap by editing `CLAUDE_EXTRA_FLAGS` and the dispatch invocation.
...
2. Check deps: `git`, `gh` (must be logged in via `gh auth login`), `tmux`, `jq`, `flock`, `systemctl`, plus the worker CLI (default `claude`). Each must resolve via `command -v`.
3. Run:
   ```bash
